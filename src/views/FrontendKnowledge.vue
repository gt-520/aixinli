...
import { addEmotionDiary, getKnowledgeList } from '@/api/frontend'
...
// 获取封面图片
const defaultCoverUrl = new URL('@/assets/images/book.png', import.meta.url).href

const getImage = (url) => {
    if (!url) return defaultCoverUrl
    return url.startsWith('http') ? url : fileBaseUrl + (url.startsWith('/') ? url : '/' + url)
}
